...
echo -e "${YELLOW}[1/7] Frontend build ediliyor...${NC}"
cd apps/web
pnpm build
cd ../..

echo -e "${YELLOW}[2/7] Backend build ediliyor...${NC}"
cd apps/api
pnpm build
cd ../..

echo -e "${YELLOW}[3/7] Sunucuda klasörler hazırlanıyor...${NC}"
ssh $SERVER "mkdir -p $REMOTE_WEB_PATH $REMOTE_API_PATH/dist $REMOTE_NGINX_PATH/conf"

echo -e "${YELLOW}[4/7] Dosyalar sunucuya yükleniyor (tar ile)...${NC}"
...
echo -e "${YELLOW}[5/7] Nginx config güncelleniyor...${NC}"
...
echo -e "${YELLOW}[6/7] Backend bağımlılıkları kuruluyor ve PM2 başlatılıyor...${NC}"
ssh $SERVER << 'ENDSSH'
...
echo -e "${YELLOW}[7/7] Nginx yeniden başlatılıyor (docker-compose)...${NC}"
ssh $SERVER << 'ENDSSH'
cd /root/nginx
