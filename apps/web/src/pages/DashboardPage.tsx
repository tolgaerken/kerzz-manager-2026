    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs md:text-sm text-muted">Lead, teklif ve satış performansı</p>
        </div>
        <button
          onClick={() => refetch()}
          disabled={isFetching}
          className="flex h-9 w-9 sm:w-auto items-center justify-center rounded-lg border border-border bg-surface px-3 text-muted transition-colors hover:bg-surface-hover hover:text-foreground disabled:opacity-50"
        >
        ...
        <div className="rounded-lg border border-border bg-surface p-4 md:p-6 md:col-span-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {!isAdmin && !isFinance && !isManager && (
              <span className="text-xs md:text-sm text-muted">Standart kullanıcı</span>
            )}
          </div>
        </div>
      </div>
