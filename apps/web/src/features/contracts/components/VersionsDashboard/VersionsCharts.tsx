...
interface VersionsChartsProps {
  stats: VersionsStats;
}

export function VersionsCharts({ stats }: VersionsChartsProps) {
  const typeData = useMemo(() => {
    const base = stats.typeDistribution;
    if (base.length <= 6) {
      return base.map((item) => ({ name: item.name, value: item.count }));
    }
    const primary = base.slice(0, 5);
    const restCount = base.slice(5).reduce((sum, item) => sum + item.count, 0);
    return [...primary.map((item) => ({ name: item.name, value: item.count })), { name: "Diger", value: restCount }];
  }, [stats.typeDistribution]);

  const currencyData = useMemo(
    () => [
      { name: "TL", value: stats.totalByPrice.tl },
      { name: "USD", value: stats.totalByPrice.usd },
      { name: "EUR", value: stats.totalByPrice.eur },
    ].filter((i) => i.value > 0),
    [stats.totalByPrice]
  );
