...
interface HardwareProductsPaginationProps {
  pagination: PaginationInfo;
  onPageChange: (page: number) => void;
  onLimitChange: (limit: number) => void;
}

export function HardwareProductsPagination({
  pagination,
  onPageChange,
  onLimitChange
}: HardwareProductsPaginationProps) {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--color-border)] bg-[var(--color-surface)]">
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        >
          {HARDWARE_PRODUCTS_CONSTANTS.PAGE_SIZE_OPTIONS.map((size) => (
