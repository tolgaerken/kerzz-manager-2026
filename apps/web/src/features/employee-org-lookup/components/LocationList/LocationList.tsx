...
import type { OrgLocation, OrgLookupQueryParams } from "../../types";
...
export function LocationList() {
  ...
  const [selectedItem, setSelectedItem] = useState<OrgLocation | null>(null);

  const { data, isLoading, refetch } = useOrgLocations(queryParams);
  const createMutation = useCreateOrgLocation();
  const updateMutation = useUpdateOrgLocation();
  const deleteMutation = useDeleteOrgLocation();

  const locations = data?.data || [];

  const handleEdit = useCallback((item: OrgLocation) => {
  ...
  const handleDelete = useCallback(
    async (item: OrgLocation) => {
      if (!confirm(`"${item.name}" lokasyonunu silmek istediğinize emin misiniz?`)) {
        return;
      }
  ...
  const columns: GridColumnDef<OrgLocation>[] = useMemo(
    () => [
      {
        id: "name",
        header: "Lokasyon Adı",
    ...
    <Box>
      ...
      <div style={{ height: 500 }}>
        <Grid<OrgLocation>
          ...
          height="100%"
          locale="tr"
          stateKey="org-locations-grid"
          getRowId={(row) => row._id}
          onRowDoubleClick={handleEdit}
          toolbar={{
            customButtons: toolbarButtons,
            exportFileName: "lokasyonlar",
          }}
        />
      </div>
