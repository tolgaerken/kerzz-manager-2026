...
import type { PaymentLinkPagination as PaymentLinkPaginationType } from "../../types/payment.types";

interface PaymentLinksPaginationProps {
  pagination: PaymentLinkPaginationType;
  onPageChange: (page: number) => void;
  onLimitChange: (limit: number) => void;
}

export function PaymentLinksPagination({
  pagination,
  onPageChange,
  onLimitChange
}: PaymentLinksPaginationProps) {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--color-border)] bg-[var(--color-surface)]">
      <div className="flex items-center gap-2 text-sm">
        ...
        >
          {PAYMENTS_CONSTANTS.PAGE_SIZE_OPTIONS.map((size) => (
